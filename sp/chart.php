...
go_to_menu($number, $school_date, $login_time, $type);
	...
	$lec = $_SESSION['lecture'];
        ...
        if ($conn) {
                ...
                $id = $_SESSION['studentid'];
		//講義ごとの提出回数を求めるSQL文　　　　　　　　　　　　　　
		$sql = "SELECT `number`,`school_date`,`present` FROM `qanda`
            WHERE `delete_flag`!='1' AND `number`='{$id}' AND `lecture`='{$lec}' GROUP BY `school_date`";
    ...
    $i=0;
    $j=1;
     while($row6=mysqli_fetch_object($query6)) {
...
*/
{
 ...
 <br> <br>
<center>
  <button class="btn" type="button" onclick="location.href='top.php'" style="width: 50%; height: 100px;  font-size: 50px;">メニューへ戻る</button>
</center>
<br><br>
