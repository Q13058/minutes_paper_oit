...
echo<<<HTML
				<thead>
				<tr>
					<th width = "50%" style = "background:#bff0ff;">{$row2_1->question3}</th>
...
HTML;
		...
		while($row=mysql_fetch_object($query)) {
			if($row->question!='未回答' && $row->question!='未登録' && !empty($row->question)){
				...
				$sql3 = "SELECT `school_date`, `number`, `rgb`, `column_color`  FROM `color` WHERE `school_date`='{$row->school_date}' AND `column_color`='{$row->reply}'  ORDER BY `school_date`";
				...
				switch ($num){
					//case "4":
					//echo<<<HTML
					//<tr><th width = "50%" style = "{$color_background}">{$row->question}</th><td width = "50%">{$row->answer}</td></tr>
					//HTML;
					//break;
...
echo<<<HTML
				...
				<tr>
					<th width = "50%" style = "{$color_background} text-align: left;">{$row->comment}</th>
					<td width = "50%" style = "background-color:#{$color};">{$row->reply}</td>
				</tr>
