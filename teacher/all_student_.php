...
$user_csv = "./user_data/" . $_SESSION['userid']. ".csv";
...
<body>
  ...
  <div id="menu"><font size=6><?php echo $studentid; ?>のミニッツペーパー</font></div>
	<div class="container">
  <br>
	<form action="" method="GET">
		...
		if ($conn) {
			...
			// データベースへの問い合わせSQL文
			$sql = "SELECT `num`, `number`, `question`, `answer`, `comment`, `reply`, `column1`, `column2`, `column3`, `column4`,`school_date`,`present` FROM `qanda` WHERE `delete_flag`!='1' AND `number`='{$studentid}' AND `lecture`='{$lec}' ORDER BY school_date DESC";
      ...
      $row4 = mysql_fetch_object($query4);
			...
			$ave =round($ave);
			if($ave>=70){
				echo '<p class="text-center"><font size="3px" color="blue">総提出率　'.$ave.' %</font></p>';
			}else{
				echo '<p class="text-center"><font size="3px" color="red">総提出率　'.$ave.' %</font></p>';
			}
...
?>
  ...
  <form action="all_student.php" method="post" enctype="multipart/form-data">
    <button class="btn btn-primary" style="font-size: 20px; position: relative; left: 0px; top: 8px; padding: 7px 80px; text-align:center">ミニッツペーパー一覧画面に戻る</button>
  </form>
</div>
<br><br>
<br><br>
