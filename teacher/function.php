...
define("USERNAME", "root_mp");
define("USERPASSWORD","mkgkJ!66");
define("DATABASE", "mpaper_Q13058");
/*
define("IPADDRESS", "127.0.0.1");
define("USERNAME", "root_mp1");
define("USERPASSWORD","mkgkJ!67");
define("DATABASE", "mpaper");
...
function error($error_number){
	if($error_number !== NULL){
		...
		if($error_number == 12){
			$_SESSION['mseg']="ファイルが選択されていません。";
		header('Location: ./file_upload.php');
	}else if($error_number == 13){
					$_SESSION['mseg']="拡張子が『.xlsx』のファイルではありません。";
				header('Location: ./file_upload.php');
	}else if($error_number == 14){
			$_SESSION['mseg']="日付が入力されていません。";
		header('Location: ./file_upload.php');
	}else if($error_number == 15){
				$_SESSION['mseg']="日付が8桁の半角数字ではありません。";
			header('Location: ./file_upload.php');
			}else{
...
function go_to_test($num, $time, $que, $ans, $com, $rep, $date, $lec, $column1, $column2, $column3, $column4, $count_num, $filename,$present) {
	...
	function go_to_color($date ,$num, $rgb, $column, $time, $lec) {
        //MySQLへの接続
        $conn = mysqli_connect(IPADDRESS, USERNAME, USERPASSWORD, DATABASE);
        if ($conn) {
		...
		//print_r($time);
                ...
                $db_selected = mysqli_select_db($conn, DATABASE);
								if($rgb == "FFFFFF"){
									...
									$rgb = hexdec($rgb);
                // データベースへの書き込みSQL文
                $sql = "INSERT INTO color(school_date,number,rgb,column_color,time,lecture) VALUES('{$date}','{$num}','{$rgb}','{$column}','{$time}','{$lec}')";
     						//print_r($sql);
                // mb_internal_encoding('sjis');
                // SQL文の実行
                $query = mysqli_query($conn,$sql);
...
/*
function go_to_sql_color($num, $time, $que, $ans, $com, $rep, $date, $lec, $column1, $column2, $column3, $column4, $filename, $lec) {
	//MySQLへの接続
	$conn = mysqli_connect(IPADDRESS, USERNAME, USERPASSWORD, DATABASE);
	if ($conn) {
		// データベースの選択
...
/*
echo '$num(number)<br>'.$num."<br>";
echo '$time(time)<br>'.$time."<br>";
echo '$que(question)<br>'.$que."<br>";
echo '$ans(answer)<br>'.$ans."<br>";
echo '$com(comment)<br>'.$com."<br>";
echo '$rep(reply)<br>'.$rep."<br>";
echo '$date(school_date)<br>'.$date."<br>";
echo '$lec(lecture)<br>'.$lec."<br>";
echo '$column1(column1)<br>'.$column1."<br>";
echo '$column2(column2)<br>'.$column2."<br>";
echo '$column3(column3)<br>'.$column3."<br>";
echo '$column4(column4)<br>'.$column4."<br>";
echo '$filename(filename)<br>'.$filename."<br>";
echo '$count_num(num)<br>'.$count_num."<br>";
echo '$present(present)<br>'.$present."<br>";
echo '$teacher(teacher)<br>'.$teacher."<br>";
*/
	//MySQLへの接続
	$conn = mysqli_connect(IPADDRESS, USERNAME, USERPASSWORD, DATABASE);
	$delete_flag = 0;
	if ($conn) {
		...
		$db_selected = mysqli_select_db($conn, DATABASE);
		// データベースへの書き込みSQL文
		$sql = "INSERT INTO qanda(number, time, question, answer, comment, reply, column1, column2, column3, column4, school_date, lecture, file_name, num, present, teacher, delete_flag) VALUES('{$num}', '{$time}', '{$que}', '{$ans}', '{$com}', '{$rep}', '{$column1}', '{$column2}', '{$column3}', '{$column4}', '{$date}', '{$lec}', '{$filename}','{$count_num}' ,'{$present}','{$teacher}',{$delete_flag})";
		// echo "<br>QandA ".$sql. "<br><br>";
		// mb_internal_encoding('sjis');
...
#question
function go_to_question_sql($date, $q0, $q1, $q2, $q3, $q4, $q5, $q6, $q7, $count, $lec) {
	/*確認用
	echo 'Q.$date(school_date)<br>'.$date."<br>";
	echo 'Q.$q0(question1)<br>'.$q0."<br>";
	echo 'Q.$q1(question2)<br>'.$q1."<br>";
	echo 'Q.$q2(question3)<br>'.$q2."<br>";
	echo 'Q.$q3(question4)<br>'.$q3."<br>";
	echo 'Q.$q4(question5)<br>'.$q4."<br>";
	echo 'Q.$q5(question6)<br>'.$q5."<br>";
	echo 'Q.$q6(question7)<br>'.$q6."<br>";
	echo 'Q.$q7(question8)<br>'.$q7."<br>";
	echo 'Q.$count(count)<br>'.$count."<br>";
	echo 'Q.$lec(lecture)<br>'.$lec."<br>";
	*/
	//MySQLへの接続
	$conn = mysqli_connect(IPADDRESS, USERNAME, USERPASSWORD, DATABASE);
	if ($conn) {
		...
		// データベースへの書き込みSQL文
		$sql = "INSERT INTO question(school_date, question1, question2, question3, question4, question5, question6, question7, question8, count, lecture) VALUES('{$date}', '{$q0}', '{$q1}', '{$q2}', '{$q3}', '{$q4}', '{$q5}', '{$q6}', '{$q7}' ,'{$count}' ,'{$lec}')";
		// echo "<br>QUESTION ".$sql. "<br><br>";
		// mb_internal_encoding('sjis');
...
//function go_to_login($count_student, $num, $school_date, $login_time, $type) {
	function go_to_login($num, $school_date, $login_time, $type) {
        //MySQLへの接続
        $conn = mysqli_connect(IPADDRESS, USERNAME, USERPASSWORD, DATABASE);
        if ($conn) {
                // データベースの選択
                ...
                // データベースへの書き込みSQL文
                //$sql = "INSERT INTO login_counter(count_student, num, school_date, login_time, type) VALUES('{$count_student}', '{$num}', '{$school_date}', '{$login_time}', '{$type}')";
								$sql = "INSERT INTO login_counter(num, school_date, login_time, type) VALUES('{$num}', '{$school_date}', '{$login_time}', '{$type}')";
...
//ログイン時の履歴の記録
function go_to_menu($num, $school_date, $used_time, $type) {
			//MySQLへの接続
			$conn = mysqli_connect(IPADDRESS, USERNAME, USERPASSWORD, DATABASE);
			if ($conn) {
							// データベースの選択
							...
							// データベースへの書き込みSQL文
							$sql = "INSERT INTO menu_counter(num, school_date, used_time, type) VALUES('{$num}', '{$school_date}', '{$used_time}', '{$type}')";
