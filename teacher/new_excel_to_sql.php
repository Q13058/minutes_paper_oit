...
//echo '<br><br>$uploadfile='.$uploadfile."<br><br>";
...
echo '</div><div id="menu">Excelファイルアップロード</div>';

/*
echo<<<HTML
<form action='top.php' method='post' enctype='multipart/form-data'>
	<button class="btn btn-primary" style="font-size: 20px; position: relative; left: 10px; top: 16px;text-align:center; text-align:center;">ミニッツペーパー編集画面に戻る</button>
</form>
HTML;
*/
	...
	for($j = 0; $j < $rowcount; $j++){//1行ずつ取得
		...
		*/
		if(!$flag){
			...
			$o=0;
 			for($p=1;$p<=$highestRow;$p++){
      	for($q=0;$q<$highestColumnIndex;$q++){
      		$color=$cell[$o];
      		$rgb[$p][$q]=$book->getActiveSheet()->getStyle($color)->getFill()->getStartColor()->getRGB();
      		//var_dump($color);
      		$o++;
      	}
      }

			for($n=0;$n<$arg_num-2;$n++){
			//var_dump($time);
			//var_dump($num);
			//var_dump($rgb[$m][$n]);
			//var_dump($column);
			...
			//echo "NUM=".$num."<br>";
			//echo "RGB=".$rgb[$m][$n+3]."<br>";
			//echo "(m,n)=(".$m.",".$n.")<br>";
			//echo "column=".$column[$n]."<br>";
			//echo "n=".$n."<br>";
		  			go_to_color($date, $num, $rgb[$m][$n+3], $column[$n], $time, $lec);
			}
			$m++;
			...
			go_to_sql($num, $time, $column[0], $column[1], $column[2], $column[3], $date, $lec, $column[4], $column[5], $column[6], $column[7],$outputName,$count_num,$present,$teacher);
		}

		if($flag&&$count_question==0){
			$flag = false;
			$count_question = 1;
			//$flag_color = false;
			for($syoki = 0; $syoki < 8; $syoki++){
			...
			$date = $_SESSION['day'];
			#データベースへの記録
			go_to_question_sql($date, $q[0], $q[1], $q[2], $q[3], $q[4], $q[5], $q[6], $q[7], $count_num, $lec);
		}else if($count_question==1&&$flag==true){
			$flag = false;
		}
...
//colorコードの被った行を削除
$sql_delete_NULL = 'DELETE FROM color WHERE lecture = "'. $_SESSION['lecture'] . '" AND school_date = "' . $date . '" AND column_color = ""';
//DELETE FROM color WHERE lecture = "プログラミング言語論（ＩＣ科３限）" AND school_date = "2016-10-11" AND column_color = ""
$query_delete = mysql_query($sql_delete_NULL, $conn);
//echo $sql_delete_NULL;
}
